...
export default function Products({ products }: { products: Products[] }) {
  ...
  return (
    <div className='relative max-w-full mx-auto top-20 py-20'>
      <div className='md:grid md:grid-cols-4 md:gap-8 gap-6 text-white'>
        <div className="md:col-span-1 min-h-screen">
          <Search />
        </div>
        {products.length > 0 ?

          <ul className='flex flex-wrap gap-4 md:col-span-3 p-3 justify-center lg:justify-start'>
            {products.map((pre, index) => {
              return (
                <li key={index} className="group relative bg-opacity-30 bg-second py-5 rounded-lg overflow-hidden shadow-lg transition-all hover:shadow-xl cursor-pointer">
                  {pre.promotion &&
                    <span className='absolute z-30 text-white font-bold flex items-center top-1 left-1 bg-primer justify-center rounded-full h-10 w-10'>-{pre.promotion}%</span>
                  }
                  <div>
                    <div>
